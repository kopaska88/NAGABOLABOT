...
async def add_admin(pool, uid:int)->bool:
    try:
        ...
        log.warning("add_admin fail %s: %s", uid, e); return False
...
async def stats_cmd(update:Update, context:ContextTypes.DEFAULT_TYPE):
    pool=context.application.bot_data["pool"]; total=await count_users(pool)
...
# -------- Broadcast flow --------
@dataclass
class BroadcastDraft:  # redefine to keep scope clear
    text:str=""; photo_file_id:Optional[str]=None; video_file_id:Optional[str]=None; animation_file_id:Optional[str]=None
    buttons:List[ButtonDef]=field(default_factory=list)
sessions = {}  # rebind to ensure clean
...
def build_app():
    ...
    # -2: debug (block=False), -1: track (block=False), 0: commands/callbacks, 1: message fallback
    app.add_handler(TypeHandler(Update, debug_all), group=-2, block=False)
    app.add_handler(TypeHandler(Update, track), group=-1, block=False)
